...
if [ ! -f "NVIDIA-Linux-x86_64-${NVID_VER}-diagnostic.run" ]; then
    echo "Driver installer file NVIDIA-Linux-x86_64-${NVID_VER}-diagnostic.run not found"
    exit 1
fi
...
chmod 755 ./NVIDIA-Linux-x86_64-${NVID_VER}-diagnostic.run

# extract nvidia files
if ./NVIDIA-Linux-x86_64-${NVID_VER}-diagnostic.run --extract-only ; then
...
cd NVIDIA-Linux-x86_64-${NVID_VER}-diagnostic
...
cd ..
if mv NVIDIA-Linux-x86_64-${NVID_VER}-diagnostic/* $DEST_DIR ; then
